...
export default function AdminAreas() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEditingId(null);
      fetchAreas();
      setFormData({ name: '', pincode: '', latitude: 0, longitude: 0, radius_km: 10, is_active: true });
    } catch (err: any) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                <div className="relative h-[300px] md:h-[400px] rounded-[2rem] overflow-hidden border-4 border-slate-50 shadow-inner group">
                  <LocationPicker 
                    ...
                    onLocationSelected={({ lat, lng, pincode }) => {
                      setFormData(prev => ({
                        ...prev,
                        latitude: lat,
                        longitude: lng,
              ...
              <form onSubmit={handleSave} className="space-y-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Area Name (e.g. Sopore Central)</label>
                  <input 
                    required
                    value={formData.name}
                    ...
                    placeholder="e.g. Sopore North Zone"
                  />
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {areas.map(area => (
          <div key={area.id} className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-slate-100 transition-all group overflow-hidden relative">
            ...
            <h3 className="text-xl font-black text-slate-900 mb-1">{area.name}</h3>
